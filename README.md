...
## 🗂️ Directory Structure (Contracts Layer Only)

contracts/
├── Anchor.toml
├── Cargo.toml
├── Xargo.toml                     # Optional: For Solana SBF builds
├── programs/
│   └── rugged-nft/
│       ├── Cargo.toml
│       └── src/
│           ├── lib.rs            # Entry point for your program
│           ├── context.rs        # Anchor contexts (structs for instructions)
│           ├── instructions/
│           │   ├── mint_rugged_nft.rs
│           │   ├── mint_standard_nft.rs
│           │   ├── update_traits.rs
│           │   └── verify_rugged_user.rs
│           ├── error.rs          # Custom program errors
│           ├── state.rs          # Custom account data structs
│           ├── utils.rs          # Utility functions (e.g. trait validation)
│           ├── constants.rs      # Seeds, tags, fee basis points, etc
│           └── cpi.rs            # Wrapper for mpl-core CPI interactions
├── migrations/
│   └── deploy.ts                 # Anchor deployment script
├── tests/
│   ├── rugged_nft.ts             # Main integration test
│   ├── helpers/
│   │   └── mpl_core_helpers.ts   # Reusable mint, update, CPI tools for tests
│   └── rugged_user_flow.ts
└── target/                       # Generated by Anchor
