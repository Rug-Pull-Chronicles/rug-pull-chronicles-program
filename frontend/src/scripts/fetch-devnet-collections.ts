...
import { IDL } from '../lib/types/rug_pull_chronicles_program';
...
async function main() {
    ...
    try {
        ...
        if (!configAccount) {
            console.error('Config account not found on devnet. Program needs to be initialized first.');
            return;
        }

        console.log('Config account found with data size:', configAccount.data.length);

        // Create a provider with no wallet (read-only)
        const provider = new anchor.AnchorProvider(
            connection,
            // @ts-ignore - using null wallet for read-only operations
            { publicKey: PublicKey.default, signTransaction: async () => { }, signAllTransactions: async () => { } },
